...
是从62开始保存，若为消极相应则从7F开始保存。
*************************************************/
int rapid_send_and_receive_cmd_by_iso_15765( const int* piCmdIndex )
{
	int iStatus = TIME_OUT;
	int i = 0;
	int iCmdSum = piCmdIndex[0];
	...
	for( i = 0; i < iCmdSum; i++ )
	{
		iStatus = rapid_send_and_receive_single_cmd_by_iso_15765( ( STRUCT_CMD * )&stSendCmd, piCmdIndex[1 + i], cReceiveBuffer );
...
Others:	按照ISO15765协议处理
*************************************************/
int rapid_send_and_receive_single_cmd_by_iso_15765( STRUCT_CMD *pstSendCmd, byte cCmdIndex, byte* pcReceiveBuffer )
{
	int		i = 0;
	int		j = 0;
	int		iReceiveStatus = TIME_OUT;
	...
	if( bMultiFrame )
	{
		...
		while( 1 )
		{
			...
			iReceiveStatus = rapid_process_CAN_receive_Cmd( cBufferOffset, pcReceiveBuffer );
...
数据的地址；
*************************************************/
int rapid_process_CAN_receive_Cmd( const byte cBufferOffset, byte* pcSource )
{
	int	iReceiveStatus	= FAIL;
	bool	bSendStatus		= false;

	uint16	u16Code7F78Timeout	= 4000;
	uint16	u16ECUResTimeout	= 4000;
	byte	cReserved			= 0xAA;
