...
bool write_Function_S( byte * cSpecialCmdData, const STRUCT_CHAIN_DATA_INPUT* pstParam, void * pOut )
{
	...
	case 1:  //限定输入起始三个字符为“LNB”
		...
		break;
		//case 2:  //主要为限制手动输入编程日期的限定
		//	{
		//		for( i = 0; i < pstParam->iLen; i++ )
		//		{
		//			if( !( pstParam->pcData[i] >= '0' && pstParam->pcData[i] <= '9' ) )
		//			{
		//				general_return_status( INVALID_INPUT, NULL, 0, pOut );
		//				return false;
		//			}
		//		}
		//		//限定月和日的写入范围
		//		if ( pstParam->pcData[2] <= '1' && pstParam->pcData[4] <= '3' )
		//		{
		//			if ( '1' == pstParam->pcData[2] )
		//			{
		//				if ( pstParam->pcData[3] >= '3' )
		//				{
		//					general_return_status( INVALID_INPUT, NULL, 0, pOut );
		//					return false;
		//				} 
		//			} 
		//			if ( '3' == pstParam->pcData[4] )
		//			{
		//				if ( pstParam->pcData[5] >= '2')
		//				{
		//					general_return_status( INVALID_INPUT, NULL, 0, pOut );
		//					return false;
		//				}
...
bool write_function( byte* cSpecialCmdData, const STRUCT_CHAIN_DATA_INPUT* pstParam, void* pOut )
{
	...
	switch( cSpecialCmdData[3] ) /* 进入扩展层，带有安全访问 */
	{
		case 1://发送安全进入命令后发送安全算法
			process_single_cmd_without_subsequent_processing( g_iExtendSessionCmdIndex[1], pOut );
			//break;
		case 2://只发送安全算法
			process_security_access_algorithm( 0, pOut );
			break;
		default:
			break;
	}
